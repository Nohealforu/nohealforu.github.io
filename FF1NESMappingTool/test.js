...
'FiendsTempleFire': new DungeonInfo('FiendsTempleFire', fiendsTempleMapTileAtlas, 'tiles_fiendsrevisited', 'tiles_fiendsrevisitedroom', 'FiendsTempleFire', new teleportEntry('FiendsTempleFireExitTarget', 'WARP', 0x0, 0x0), [0xA8, 0xA8, 0xA6, 0xA6, 0xA9, 0xA9, 0xA7, 0xAA], 10),
...
{
	let polarity = (direction == Directions.Down || direction == Directions.Right) ? 1 : -1
	let xMotion = (direction == Directions.Right || direction == Directions.Left) ? 1 : 0;
	let yMotion = (direction == Directions.Down || direction == Directions.Up) ? 1 : 0;
	
	return {x: this.x + xMotion * polarity, y: this.y + yMotion * polarity};
}

EncounterTrackerTile.prototype.getTileData = function (direction)
{
	let newCoords = this.getNewCoords(direction);
...
EncounterTrackerTile.prototype.getPotentialAdjacentMovementType = function (tileData, tileX, tileY)
{
	...
	else if(this.moveMethod == MoveMethod.Canoe && tileData.canoe == false)
	{
		...
		else if(this.map.overworldMap && Game.ship.active == true && Game.ship.gridX == tileX && Game.ship.gridY == tileY)
		{
			return MoveMethod.Ship;
		}
		else
	...
	else if(this.moveMethod == MoveMethod.Walk && !this.map.overworldMap)
	{
		return MoveMethod.Walk;
	}
	else if(this.moveMethod == MoveMethod.Ship && this.map.overworldMap && Game.canal.active == true && Game.canal.gridX == tileX && Game.canal.gridY == tileY)
		return -1;
...
EncounterTrackerTile.prototype.checkForIncrementEncounter = function (tileData, moveMethod, teleported, currentDomain)
{
	...
	if(moveMethod != this.moveMethod || teleported || 
	   !((moveMethod == MoveMethod.Walk && tileData.fight == worldMapTileFight.Fight) ||
	   (moveMethod == MoveMethod.Canoe && tileData.fight == worldMapTileFight.FightRiver) ||
	...
	if(encounterInfo.stepCounter2 < 0x80)
		encounterInfo.stepCounter1++;
	else
		encounterInfo.stepCounter1--;
	if(encounterInfo.stepCounter1 < 0)
		encounterInfo.stepCounter1 += 0x100;
	else if(encounterInfo.stepCounter1 > 0xFF)
		encounterInfo.stepCounter1 -= 0x100;
	if(encounterInfo.stepCounter1 == 0)
		encounterInfo.stepCounter2 += 0xA0;
	if(encounterInfo.stepCounter2 > 0xFF)
		encounterInfo.stepCounter2 -= 0x100;
	encounterInfo.encounterChance = encounterChanceTable[encounterInfo.stepCounter1];
	let encounterThreshold = (this.moveMethod == MoveMethod.Ship ? 3 : this.map.encounterThreshold);
	if(encounterInfo.encounterChance < encounterThreshold)
	{
		encounterInfo.encounterGroup = encounterGroupTable[encounterInfo.encounterNumber];
		
		encounterInfo.encounterId = 0;
		if(this.map.overworldMap)
		{
		...
		if(encounterInfo.encounterNumber > 255)
			encounterInfo.encounterNumber -= 256;
	}
...
Game.init = function () {
	...
	this.encounterCanvas = document.createElement('canvas');
    this.encounterCanvas.width = defaultWidth;
    this.encounterCanvas.height = defaultHeight;
	...
	this._drawEncounters(this.currentMap);
    console.log("Intial Encounter Draw Complete");
    this._drawSprites(this.currentMap);
...
Game.takeEncounterStep = function()
{
...
Game.takeCanalGlitchStep = function()
{
	this.incrementStepCounter();
...
Game.updateEncounterTracker = function ()
{
	...
	if(!this.currentMap.overworldMap && this.currentDungeon.warpInformation.length > 0)
	{
		...
		visitedMapCounts[this.currentDungeon.label] = 1;
		while(teleport.targetMap != 'WorldMap' && teleport.targetMap != 'WARP' && recursions++ < this.encounterTrackerDistance)
		{
			let targetMap = teleport.targetMap;
			if(visitedMapCounts[targetMap] == null)
				visitedMapCounts[targetMap] = 0;
	...
	if(this.possibleEncounterNumbers.length > 0)
	{
		...
		for(let i = 0; i < this.possibleEncounterNumbers.length; i++)
		{
			...
			for(const encounterId in encounterIdsWithData)
			{
				...
				if(fightNumber > 127)
					fightNumber = (fightNumber - 128) + '-2';
				encounterStringGroup.push('Fight: ' + fightNumber + ' surprise(' + encounter.surprise +') can run(' + encounter.runnable + '): ' + fightDetails);
...
{
	if(currentEncounterTile.steps == this.encounterTrackerDistance)
		return;
	this.queueAdjacentTrackerTile(currentEncounterTile, Directions.Up);
	this.queueAdjacentTrackerTile(currentEncounterTile, Directions.Left);
	this.queueAdjacentTrackerTile(currentEncounterTile, Directions.Down);
	this.queueAdjacentTrackerTile(currentEncounterTile, Directions.Right);
};

Game.queueAdjacentTrackerTile = function(currentEncounterTile, direction)
{
	let encounterTileMap = currentEncounterTile.map;
	let currentDomain = currentEncounterTile.overworldMap ? null : Math.floor(currentEncounterTile.x / 32) + Math.floor(currentEncounterTile.y / 32) * 8; 
	let newCoords = currentEncounterTile.getNewCoords(direction);
	let tileData = currentEncounterTile.getTileData(direction);
	...
	{
		if((teleport.requirement == teleportEntryRequirement.Crown && this.player.keyItems[KeyItem.CROWN] == false)
		||(teleport.requirement == teleportEntryRequirement.Cube && this.player.keyItems[KeyItem.CUBE] == false)
		||(teleport.requirement == teleportEntryRequirement.Chime && this.player.keyItems[KeyItem.CHIME] == false)
		||(teleport.requirement == teleportEntryRequirement.Orbs && this.player.getOrbs() == false))
			return;
		else //check teleport location
		{
			...
			else
			{
				...
				encounterTileMap = {
					...
					maxCol: 2 * dungeonCells.cols,
					maxRow: 2 * dungeonCells.rows,
					data: Loader.getMapData(dungeonInfo.mapDataName),
					mapTileAtlas: dungeonInfo.mapTileAtlas,
					tileAtlasImage: [],
					loadRooms: true,
					showRooms: false,
					overworldMap: false,
...
Game._drawEncounters = function (map) {
    let context = this.encounterCanvas.getContext('2d');
    context.imageSmoothingEnabled = false;
    context.clearRect(0, 0, defaultWidth, defaultHeight);
	context.lineWidth = 4;
    ...
    for (let i = 0; i < this.possibleEncounters.length; i++) 
	{
		...
		if(encounter.map.name == map.name)
		{
			if(startCol <= encounter.x && endCol >= encounter.x &&
			   startRow <= encounter.y && endRow >= encounter.y)
			{
